...
const TopQualitySection = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await getTopRestaurants("diemChatLuong", 10);
        setRestaurants(data);
      } catch (error) {
        console.error("Failed to fetch top quality:", error);
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className="container">
        <div className="section-header-row">
          <div>
            ...
            <p className="section-subtitle">{T.home.qualitySub}</p>
          </div>
                ...
                <div key={res._id} className="rating-card clickable" onClick={() => handleCardClick(res)}>
                    ...
                    <div className="card-content">
                      ...
                      <p className="card-tag-row">
                        <span className="card-category">{T.restaurantPage.labels.quality} <strong>{formatRating(res.diemChatLuong)}</strong></span>
