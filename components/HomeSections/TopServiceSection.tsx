...
// Icons... (Giữ nguyên các SVG như trên)
...
const TopServiceSection = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await getTopRestaurants("diemPhucVu", 10);
        setRestaurants(data);
      } catch (error) {
                ...
                <div key={res._id} className="rating-card clickable" onClick={() => handleCardClick(res)}>
                    <div className="card-image-wrapper">
                      ...
                      <div className="rating-badge" style={{ backgroundColor: '#6c5ce7' }}>
                        <ScoreBadge score={res.diemPhucVu} />
                      </div>
                    </div>
                    <div className="card-content">
                      <h3 className="restaurant-name">{res.tenQuan}</h3>
                      <p className="card-tag-row">
                        <span className="card-category">{T.restaurantPage.labels.service} <strong>{formatRating(res.diemPhucVu)}</strong></span>
            ...
            <div className="modal-panel" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="modal-layout">
                <div className="modal-image-col">
                  ...
                  <div className="modal-rating-overlay" style={{background: 'rgba(108, 92, 231, 0.9)'}}>
                    <span className="big-score"><ScoreBadge score={selectedRes.diemPhucVu} withScale /></span>
                    <span className="score-label">
                      {ratingLabel(selectedRes.diemPhucVu, T)}
                    </span>
                  </div>
                </div>
                <div className="modal-info-col">
                  ...
                  <div className="rating-bars">
                    <RatingRow label={T.restaurantPage.labels.service} score={selectedRes.diemPhucVu} highlight={true} color="#6c5ce7"/>
